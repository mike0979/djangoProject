    ...
    if id == None:
        if request.method == "GET":
            ...
            if line_id == None:
                ...
                res = models.TblLine.objects.filter(line_id=line_id).values()
                return JsonResponse(list(res), safe=False)
            ...
            models.TblStation.objects.create(id=res.id, user_type=1, code=code, name=name, name_en=name_en, description=description, position=position, cross_station_ids=cross, diagram=diagram)
...
def line_operate(request, id=None):
    if id == None:
        if request.method == "GET":
            ...
            models.TblLine.objects.create(id=res.id, user_type=1, code=code, name=name, name_en=name_en, description=description, back_image=back_image)
            return JsonResponse({'id' : res.id})
    else:
        if request.method == "GET":
            res = models.TblLine.objects.filter(id=id).values().first()
